...
/*   Updated: 2024/01/16 23:40:20 by sguzman          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_line_from_list(t_list **lst, char **line)
{
	...
	*line = malloc(sizeof(char) * (len + 1));
	...
	while (aux)
	{
		while (*((char *)(*aux).content + j) && i < len)
			*(*line + i++) = *((char *)(*aux).content + j++);
		j = 0;
		aux = (*aux).next;
	}
	*(*line + i) = '\0';
...
void	extract_remainder(t_list **lst)
{
	...
	while (*((char *)(*last).content + before + len))
		len++;
	if (!len)
		return (clear_nodes(lst));
	...
	if (!content)
		return (clear_nodes(lst));
	i = 0;
	while (*((char *)(*last).content + before) && i < len)
		*(content + i++) = *((char *)(*last).content + before++);
	*(content + i) = '\0';
	clear_nodes(lst);
